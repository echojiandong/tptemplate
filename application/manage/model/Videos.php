...
class Videos extends Model{
	...
	public function videotree($g_type, $s_type){
		...
		//学科
		$subject_arr = Db::name('subject') ->field('subject') ->where(['id' => $s_type]) ->find();
		//取出所有课程的主键id
		...
		foreach($c_video as $key =>&$val){
			...
			if(isset($val['part']) && $val['part'] == 2){
				...
				if(!isset($val['part'])){
					...
					if(isset($val['edition_id']))
					{
						$c_video[$key]['edition_id'] = isset($edition_arr[$val['edition_id']]) ? $edition_arr[$val['edition_id']] : '--' ;
					}
					$c_video[$key]['subject'] = $subject_arr['subject'];
		...
		$list = array_map(function($v) use ($v_parent,$v1_parent){
					if(isset($v['part']) && $v['part'] == 0){
						...
						$v['pid'] = isset($v1_parent[$v['pid']]) ? $v1_parent[$v['pid']] : $v_parent[$v['kid']] ;
					}
					return $v;
			}, $c_video);
		//将知识点 压入数组
	...
	public function addclassinterferce1(){
		...
		$where['videoTag'] = $this ->parmes['videoTag'];

		$where['courseware'] = isset($this ->parmes['courseware']) ? $this ->parmes['courseware'] : 0;

		$where['courseware1'] = isset($this ->parmes['courseware1']) ? $this ->parmes['courseware1'] : 0;

		$where['courseware2'] = isset($this ->parmes['courseware2']) ? $this ->parmes['courseware2'] : 0;
	...
	public function addclassinterferce2(){
		...
		//  课件资料
		$data['courseware'] = isset($this ->parmes['courseware']) ? $this ->parmes['courseware'] : 0;

		$data['courseware1'] = isset($this ->parmes['courseware1']) ? $this ->parmes['courseware1'] : 0;

		$data['courseware2'] = isset($this ->parmes['courseware2']) ? $this ->parmes['courseware2'] : 0;
		//封面图
		$data['img'] = isset($this ->parmes['img']) ? $this ->parmes['img'] : '';
		//试听状态
		$data['audi'] = isset($this ->parmes['audi']) ? $this ->parmes['audi'] : '';
		//视频标签
		$data['videoTag'] = isset($this ->parmes['videoTag']) ? $this ->parmes['videoTag'] : '';
		// 试听时长
		$data['audi_time'] = isset($this ->parmes['audi_time']) ? $this ->parmes['audi_time'] : '';
		// 默认时长五分钟
		if($data['audi'] == 1 && $data['audi_time'] == ''){

			$data['audi_time'] = 300;
		}
		if(!empty($teacher[0]) && isset($teacher[1])){
			//教师id
	...
	public function curriculumedit(){
		...
		unset($this ->parmes['layuiTreeCheck']);

		$id = isset($this ->parmes['selfid']) ? $this ->parmes['selfid'] : 0;
		$this ->parmes['videoClassTag'] = isset($this ->parmes['ids']) ? trim($this ->parmes['ids'],',') : 0;
		unset($this ->parmes['ids']);
		// 教师
	...
	public function deleteVCdata($id){
		// 查询 父节点下所有数据
		$data = Db::name('video') ->field('id,kid,img,link,link_720,link_480,courseware,courseware1,courseware2') 
								  ->where(['kid' => $id]) 
								  ->select();

		$p_data = Db::name('video_class') ->field('id,courseware,courseware1,courseware2') ->find();
		//  删除课件资料
		if($p_data['courseware'] != 0){

			$this ->qiniu->delFile('ydtvlitpic',substr($p_data['courseware'],strripos($p_data['courseware'],'/')+1));
		}
		if($p_data['courseware1'] != 0){

			$this ->qiniu->delFile('ydtvlitpic',substr($p_data['courseware1'],strripos($p_data['courseware1'],'/')+1));
		}
		if($p_data['courseware2'] != 0){

			$this ->qiniu->delFile('ydtvlitpic',substr($p_data['courseware2'],strripos($p_data['courseware2'],'/')+1));
		}
	...
	public function deleteVdata($id){
		...
		//查询是否有孩子
		$p_data = Db::name('video') ->field('id,kid,img,link,link_720,link_480,courseware,courseware1,courseware2') 
										  ->where('pid','IN',$id) 
										  ->select();
		// 递归处理
		if(!empty($p_data)){
	...
	private function delqiniudata($data){
		...
		foreach($data as &$val){
			...
			//  课件资料
			if(!empty($val['courseware'])){
				$delink[] = ['spnce' => 'ydtvlitpic', 'name' => substr($val['courseware'],strripos($val['courseware'],'/')+1)];
			}
			if(isset($val['courseware1']) && !empty($val['courseware1'])){
				$delink[] = ['spnce' => 'ydtvlitpic', 'name' => substr($val['courseware1'],strripos($val['courseware1'],'/')+1)];
			}
			if(isset($val['courseware2']) && !empty($val['courseware2'])){
				$delink[] = ['spnce' => 'ydtvlitpic', 'name' => substr($val['courseware2'],strripos($val['courseware2'],'/')+1)];
			}
		}
		if(!empty($delink)){
	...
	public function videoTagTree($id,$type)
	{
		//获取视频标签
		//type:1 video_class标签 type:2 video标签
		if($type == 1){
			//获取video_class 视频标签
			$res = Db::name('video_class')->field('videoClassTag')->where(['id'=>$id])->find();
			$videoTagList = explode(',',$res['videoClassTag']);
		}elseif($type == 2){
			//获取video 视频标签
			$res = Db::name('video')->field('videoTag')->where(['id'=>$id])->find();
			$videoTagList = explode(',',$res['videoTag']);
		}
