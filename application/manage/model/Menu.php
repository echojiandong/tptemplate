...
class Menu extends Model
{
    ...
    public function getmenu(){
        $group_id = Session::get('manageinfo')['group_id'];
        $rs = tree(Db::name("menu")
                            ->alias('m')
                            ->field('m.*')
                            ->join("guard_user_group u","m.id=u.menu_id")
                            ->where('m.status = 1 and u.groupid = '.$group_id)
                            ->order("m.menuid",'asc')
                            ->select(),0);
    ...
    public function update_menu(){
        ...
        if(!empty($param['href'])){$where['href']=$param['href'];}
    ...
    //删除管理员分组
    public function delAdminGroup($where)
    {
        return Db::name('group')->where($where)->delete();
    }
}