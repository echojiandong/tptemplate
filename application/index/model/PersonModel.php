...
class PersonModel
{
    ...
    public function GetUserClass($class_id,$subject_id = 1,$semester_id = 1){
        ...
        //查询class_id课程列表
        if($subject_id>9){
            $list = Db::name('product') ->field('id,grade_id,subject_id,Semester')
        ...
        if(!empty($list)){
            ...
            //获取视频总节数
            if($subject_id>9){
                $count_video = Db::name('product_info') ->field('count(1) as counts') ->where("product_id = ".$list['id']." and forbiden = 1") ->where(['display' => 1]) ->find();
            }else{
    ...
    public function getSubjectUser($class_id){
        ...
        $subject_list = Db::name('video_class') ->field('subject_id') 
                                                ...
                                                ->group('subject_id') 
                                                ->select();
        $subject_product_list = Db::name('product') ->field('subject_id')
            ->where(['grade_id' => $class_id])
            ->group('subject_id')
            ->select();
        $subject_list = array_merge($subject_list,$subject_product_list);
    ...
    public function getMystudyClass($class_id){
        ...
        if($user){
            //获取用户购买的课程
            $SqlList = Db::name('order_person_son')
                ->where(['is_audition'=>0,'person_id'=>$user['id']])
                ->order('orderCheck','asc')
                ->limit('999')->buildSql();
            //获取video_class课程
            $CourseList = Db::table($SqlList)->alias('ps')
                            ->join('guard_video_class vc','vc.id = ps.video_class_id')
                            ->field("vc.*,ps.orderCheck,ps.id as ids")
                            ->where(['vc.grade_id'=>$class_id,'is_audition'=>0,'person_id'=>$user['id']])
                            ->group('ps.video_class_id')
                            ->select();

            $ProductList = Db::table($SqlList)->alias('ps')
            ...
            $CourseList = array_merge($CourseList,$ProductList);
        ...
        foreach($CourseList as $k=>$v)
        {
            ...
            if(!empty($user)){
                ...
                foreach ($classList as $key => $value){
                    if(($value['video_class_id'] == $v['id'] && !isset($v['productUrl'])) || ($value['product_id'] == $v['id'] && isset($v['productUrl']))){
                        if(isset($v['productUrl'])){
    ...
    public function recommendClass($class_id){
        ...
        //获取已购买产品
        $CourseList2 = Db::name('order_person_son')->alias('ps')
            ->join('guard_product vc','vc.id = ps.product_id')
            ->field("vc.id")
            ->where(['vc.grade_id'=>$class_id,'is_audition'=>0,'person_id'=>$user['id']])
            ->select();
        ...
        foreach ($CourseList2 as $key => $value) {
            $classList2[] =  $value['id'];
        }
        $noBuyClass2 = Db::name('product')
            ->field("*")
            ->where(['grade_id'=>$class_id,'id'=>array('not in',$classList2)])
            ->limit(0,12)
            ->select();
        foreach ($noBuyClass2 as $k=>$v){
            $noBuyClass2[$k]['img'] = $v['productUrl'];
            array_push($noBuyClass,$noBuyClass2[$k]);
        }
        return $noBuyClass;
    }
    //获取用户信息
    ...
    public function cardactivation($gid, $uid){
        ...
        $classlist = Db::name('video_class') ->field('subject_id,id,grade_id,Semester')
                                             ->where(['grade_id' => $gid]) 
                                             ->select();
        $classlist2 = Db::name('product') ->field('subject_id,id,grade_id,Semester')
            ->where(['grade_id' => $gid])
            ->select();
        $classlist = array_merge($classlist,$classlist2);
        ...
        $class_p_id = array_column($new_subjectlist,'p_id');
        $order = $order2 = array();
        // 查询审核通过的正式课订单
        if(!empty($class_id)){
        ...
        if(!empty($class_p_id)){
            $order2 = Db::name('order_person_son') ->field('is_audition,person_id,product_id,type')
                ->where(['person_id' => $uid])
                ->where(['orderCheck' => 2, 'is_audition' => 0, 'is_forbidden' => 0])
                ->where('product_id', 'in', $class_p_id)
                ->select();
        }

        if(!empty($order) || !empty($order2)){

            $one_order = array_combine(array_column($order, 'video_class_id'),array_column($order, 'type'));
            $one_order2 = array_combine(array_column($order2, 'product_id'),array_column($order2, 'type'));
            foreach($new_subjectlist as $key =>&$val){
                ...
                if(isset($val['p_id'])){
                    if(isset($one_order2[$val['p_id']]) && !in_array($uid, $user_vip)){

                        $new_subjectlist[$key]['is_activate'] = $one_order2[$val['p_id']] == 1 ? 2 : 1;
                    }
                }
        ...
        $active_log = Db::name('video_log') ->field('person_id,video_class_id,type,expireTime') 
                                     ...
                                     ->select();
        $active_log2 = Db::name('video_log') ->field('person_id,product_id,type,expireTime')
            ->where(['person_id' => $uid, 'type' => 0])
            ->where('product_id', 'in', $class_p_id)
            ->group('product_id,person_id')
            ->select();
        $active_log = array_merge($active_log,$active_log2);
     ...
     * @return   [int]                  
     */
    public function activeCourses($cid, $uid,$productStatus=0){

        if($productStatus==1){
            $where = ['person_id' => $uid, 'is_audition' => 0, 'product_id' => $cid, 'orderCheck' => 2];
        }else{
        ...
        if($is_true['type'] == -1){
            ...
            Db::transaction(function() use ($cid, $uid, $where,$productStatus){
                // 查询课程剩余秒数
                if($productStatus==1){
                    $surplus_time = Db::name('video_log') ->field('expireTime')
                        ->where(['person_id' => $uid, 'product_id' => $cid])
                        ->find();
                    // // 秒数不为 负(需要时 可加)
                    ...
                    Db::name('video_log') ->where(['person_id' => $uid, 'product_id' => $cid])
        ...
        // 获取主课程对应信息
        if($productStatus==1){
            $video_msg = Db::name('product') ->field('id,grade_id,subject_id,Semester,effectivedays')
                ->where(['id' => $cid])
                ->find();
        }else{
        ...
        if($productStatus==1){
            $is_renew = Db::name('order_person_son') ->field('id')
                ->where(['person_id' => $uid, 'is_audition' => 0, 'product_id' => $cid, 'orderCheck' => 4])
                ->find();
        }else{
            $is_renew = Db::name('order_person_son') ->field('id')
        ...
        if(!empty($is_renew)){
            ...
            Db::transaction(function() use ($cid, $uid, $where,$dq_time,$productStatus){
                // 查询课程剩余秒数
                if($productStatus==1){
                    $surplus_time = Db::name('video_log') ->field('expireTime')
                        ->where(['person_id' => $uid, 'product_id' => $cid])
                ...
                if($surplus_time['expireTime'] > time()){
                    ...
                    $expiretime = ((int)$surplus_time['expireTime']-time()) + $dq_time;
                }
        ...
        if(!empty($is_buyAudition)){
            ...
            $condition['audi'] = 2;
            if($productStatus==1){
                $surplus_time1 = Db::name('video_log') ->where(['person_id' => $uid, 'product_id' => $cid])
        ...
        //获取对应的 视频信息
        if($productStatus==1){
            $info_ids = Db::name('product_info') ->where(['product_id' => $cid])
                ->field('video_id')
                ->select();
            $info_id = array_column($info_ids,'video_id');
        ...
        if($productStatus==1){
            $is_upd1 = Db::name('order_person_son') ->where(['person_id' => $uid, 'is_audition' => 0, 'product_id' => $cid]) ->update(['type' => 1]);
        }else{
