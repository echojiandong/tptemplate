...
class Course extends Communal
{
    ...
    public function courseParticulars()
    {
        ...
        if(isset($param['videoid']) && !empty($param['videoid']) && $param['videoid'] != 'undefined'){
            ...
            if(!empty($info)){//用户登陆了
                ...
                $wherelog['video_class_id']=$param['id'];
                $wherelog['expireTime']=array('egt',time());
                $result = Db::name('video_log')->where($wherelog) ->where(['type' => 0]) ->find();
                ...
                if($result){
                    if($req){//用户购买过本视频并且有学习的记录有观看学习的记录比较用户上次学习记录
                        ...
                        if($a>10){//用户本节课程还没有学习完
                            ...
                            $whereUser="person_id=".$info['id']." and video_class_id=".$param['id']." and study_time =0";
                            $rtq=Db::name('video_log')->field('id,video_id,study_time,video_time') ->where(['type' => 0]) ->where($whereUser)->order('id asc')->find();
                            $whereCourse['id']=$rtq['video_id'];
                            $starttime=0;
                        } 
                    }else{//用户还未开始学习
                        $whereUser="person_id=".$info['id']." and video_class_id=".$param['id']." and study_time =0";
                        $rtq=Db::name('video_log')->field('video_id,study_time,video_time') ->where(['type' => 0]) ->where($whereUser)->order('id asc')->find();
                        if($rtq['video_id']){
                            $whereCourse['id']=$rtq['video_id'];
                        }
                        $starttime=0;
        ...
        $courseCatalogList=$this->courseCatalogList($param['id'],$info);//url地址videoid不存在，并且没有用户登陆
    ...
    public function getPayCourse($id)
    {
        if($this ->_info){
            $courseCatalogList=$this->courseCatalogList($id,$this ->_info);//url地址videoid不存在，并且没有用户登陆
            
        }else{
            $courseCatalogList=$this->courseCatalogList($id);
        }
        ...
        $info = $this ->_info;
        $isbay = Db::name('video_log')->where(['person_id' => $info['id'], 'video_class_id' => $id, 'type' => 0])->find();
    ...
    //获取课程章节目录
    public function courseCatalogList($id,$info="")
    {
        $where['kid']=$id;
        $res=Db::name('video')->where($where) ->where(['display' => 1])->order('sort')->select();
        $_personid = isset($info['id']) ? $info['id'] : 0;
        $rts=Db::name('video_log') ->where(['type' => 0,'video_class_id'=>$id,'person_id' => $_personid])->find();
        foreach($res as $k=>$v){
            if($v['part']==2){
                $whereknow['s_id']=$v['id'];
...
public function courseVerification()
{
    ...
    //  是否试听
    $datas['is_audi'] = $audi['audi'] == 1 ? 1 : 0;

    $is_buy = 0;

    if($info){
        $is_buy = Db::name('video_log') ->field('id') 
                ->where(['person_id' => $info['id'], 'video_id' => $video_id, 'type' => 0]) 
                ->find();
    }
    //  是否购买
           ...
           jsonMsg('sucess',1,$res,$datas);
            ...
            if($res['audi']==1){
                ...
                if($info['addtime'] < $time) {
                    if(empty($audiInfo) && $info['id'] > 27){
                        if($audiNum < 5){
                            jsonMsg("每个用户能免费试听<span style='color: #FF6F68;'>5</span>个视频<p class='number_p audited'>您已观看<span style='color: #FF6F68;'>".$audiNum."</span>个</p>",2,$res,$datas);
                        }else{
                ...
                jsonMsg('<p style="font-size: 24px;">该课程不属于试听课程</p><p class="number_p audited" style="padding-top: 19px">若想继续学习,请先购买</p>',0,'/index/course_pay/coursePay');
            }
        }
    }
    // else{
...
public function closeUpdateCourseLog()
{
    ...
    if($param['person_id']>=1){
       ...
       if($rts){
            ...
            $this->checkCloseBrowLog($param);
       }else{
            if(!in_array($info['id'],config('user_vip'))){
                if($param['study_time'] != '0:0' && $param['nowTime'] != '0:0'){
                    $audiInfo = Db::name('audition_log')->where(['person_id'=>$info['id'],'video_id'=>$param['videoid']])->order('id desc')->find();
                    //获取视频信息
 ...
 * @author yangjizhou 2019-06-19
 */
public function checkCloseBrowLog($param)
{
    if (!$param) {
        return false;
    }
    ...
    *手机端全局课程搜索
    ...
    public function addNote()
    {
        ...
        $data = Db::name('person_collect') ->field('id') 
                                           ->where(['uid' => $user['id'],'video_id' => $video_id,'status' => 2]) 
                                           ->find();
        if(!empty($data)){
    ...
    public function settextareaval(){
        ...
        $data = Db::name('person_collect') ->field('noteText') 
                                           ->where(['uid' => $user['id'], 'video_id' => $param['vid'], 'status' => 2]) 
                                           ->find();
        if(empty($data)){
    ...
    public function checkVideoPlay(){
        ...
        if($user){
            $is_buy = Db::name('video_log') ->field('id') 
                                  ->where(['person_id' => $user['id'], 'video_id' => $video_id, 'type' => 0]) 
                                  ->find();
        }
        //  是否购买
                            ...
                            if($info['addtime'] < $time) {
                                ...
                                if(empty($audiInfo) && $info['id'] > 27){
                                    if($audiNum >= 5){
                                        ...
                                        return jsonMsg('每个用户能免费试听<span style="color: #FF6F68;">5</span>个视频<p class="number_p audited">您已试听<span style="color: #FF6F68;">'.$audiNum.'</span>个</p>',1);
                                    }
                                }else{
                                     return jsonMsg('每个用户能免费试听<span style="color: #FF6F68;">5</span>个视频<p class="number_p audited">您已试听<span style="color: #FF6F68;">'.$audiNum.'</span>个</p>',1);
                                }
