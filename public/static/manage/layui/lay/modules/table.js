...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
                  ...
                  case 'checkbox':
                    return '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" '+ function(){
                      ...
                      if(options.cacheCheck){
                        ...
                        var str = JSON.stringify(item1);
                        return cacheCheckData[str.substring(str.indexOf(':')+1,str.indexOf(','))]?'checked':''
    ...
    Class.prototype.setCheckData = function(index, checked){
      ...
      ,cacheCheckData = table.cacheCheck[that.key]||{};
      if(!thisData[index]) return;
      if(thisData[index].constructor === Array) return;
      thisData[index][options.checkName] = checked;
      //这下面是新增加的代码 操作缓存
        if(options.cacheCheck&&checked){
            if(!table.cacheCheck[that.key]){//初始化数据
                table.cacheCheck[that.key]={};
                cacheCheckData=table.cacheCheck[that.key];
            }
            //cacheCheckData[thisData[index][options.checkKey]]=thisData[index];
            var str = JSON.stringify(thisData[index]);
            cacheCheckData[str.substring(str.indexOf(':')+1,str.indexOf(','))]=thisData[index];
        }else if(options.cacheCheck&&!checked){
            //delete cacheCheckData[thisData[index][options.checkKey]];
            var str = JSON.stringify(thisData[index]);
            delete cacheCheckData[str.substring(str.indexOf(':')+1,str.indexOf(','))];
        }
    };
    ...
    //获取所有选中状态的 
    table.checkAllStatus = function(id){
        var data=[];
        $(table.cacheCheck[id]).each(function (i, e) {
            for(let key in e){
                data.push(e[key]);
            }
        })
