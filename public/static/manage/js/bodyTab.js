...
layui.define(["element","jquery"],function(exports){
	...
	//右侧内容tab操作
	var tabIdIndex = 0;
	Tab.prototype.tabAdd = function(_this,url){
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
			    ...
			    if(_this.attr("target")=='view_window'){
			    	var href_url=url;
			    	top.location=top.location;
			    	var title_now=_this.text();
			    }else{
			    	var href_url=_this.attr("data-url");//点击左侧栏目
			    	var title_now=_this.find("cite").text();
			    }
			    element.tabAdd(tabFilter, {
				        title : title,
				        content :"<iframe src='"+href_url+"' data-id='"+tabIdIndex+"'></frame>",
				        id : new Date().getTime()
				    })
				//当前窗口内容
				var curmenu = {
