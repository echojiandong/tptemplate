...
class Api 
{
    ...
    public function __construct($config)
    {
        ...
        //@error_log('api config:'.json_encode($config),0);
    }
...
//                }
    ...
    {
        $file = './upload/qrcode/merge4383.jpg';
        $file_info=array(
            'filename'=>'merge4383.jpg',  //图片相对于网站根目录的路径
            'content-type'=>'image/jpeg',  //文件类型
            'filelength'=>filesize($file)       //图文大小
        );

        $url = self::API_DOMAIN . 'cgi-bin/media/upload?access_token=' . $this->get_access_token() . '&type=' . $type;        
        $res = HttpCurl::post($url, json_encode(array('media' => $path,'form-data'=>$file_info)), 'json');
        // 异常处理: 获取时网络错误
        if ($res === false) {
            return Error::code('ERR_GET');
        }        
    ...
    //$path服务器的绝对路径
    public function upload_media_new($type, $path)
    {
        $data['media'] = HttpCurl::getCurlFileMedia($path);

        $url = self::API_DOMAIN . 'cgi-bin/media/upload?access_token=' . $this->get_access_token() . '&type=' . $type;
        $res = HttpCurl::CurlFile($url, $data);
        // 异常处理: 获取时网络错误
        if ($res === false) {
            return Error::code('ERR_GET');
        }
    ...
    public function get_jsapi_ticket()
    {
        ...
        if (!$this->valid_jsapi_ticket($ticket)) {
            ...
            if ($this->save_jsapi_ticket_diy !== false) {
                // 用户自定义保存jsapi_ticket方法
                call_user_func($this->save_jsapi_ticket_diy, json_encode($ticket));
            } else {
                // 异常处理: 保存jsapi_ticket方法未定义
                @error_log('Not set saveTicketDiy method, jsapi_ticket will be refreshed each time.', 0);
            }
        }
